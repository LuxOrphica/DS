...
function recalcConflicts(db) {
  ...
  for (const bucket of byBrandArticle.values()) {
    ...
    let reason = 'Дубликат артикула в рамках бренда';
    if (names.size > 1) reason += ' (разные названия)';
    else if (groups.size > 1) reason += ' (разные категории)';
  ...
  for (const row of rows) {
    ...
    if (status && status !== 'normalized') {
      addNote(row.id, `Требует проверки нормализации: ${row.normalizationStatus}`);
    }
  }
