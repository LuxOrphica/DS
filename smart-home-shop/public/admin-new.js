class AdminApp {
  ...
  setupEventListeners() {
    ...
    if (productCategory) {
      productCategory.addEventListener('change', () => this.renderProductAttributeEditor());
    }
    ...
    if (ordersSearchInput) {
      ordersSearchInput.addEventListener(
        'input',
        this.debounce(() => this.applyOrdersFilters(), 300)
      );
    }
    [
      'ordersStatusFilter',
      ...
      const el = document.getElementById(id);
      if (el) el.addEventListener('change', () => this.applyOrdersFilters());
    });
  }
  ...
  async loadFilters() {
    try {
      ...
      if (groupFilter) groupFilter.innerHTML = '<option value="">\u0412\u0441\u0435 \u043f\u043e\u0434\u043a\u0430\u0442\u0435\u0433\u043e\u0440\u0438\u0438</option>';
      ...
      if (productFunctionalCategories) productFunctionalCategories.innerHTML = '';
      ...
      this.availableFilters.categories.forEach((categoryName) => {
        ...
        if (productFunctionalCategories) productFunctionalCategories.add(new Option(categoryName, categoryName));
      });
      ...
      if (productCategory && productFunctionalCategories) {
        if (!productCategory.dataset.functionalSyncBound) {
          productCategory.addEventListener('change', () => {
            const v = String(productCategory.value || '').trim();
            if (!v) return;
            this.ensureFunctionalCategorySelected(v);
          });
  ...
  syncGroupFilterOptions() {
    ...
    const allGroups = Array.from(new Set((this.availableFilters.groups || []).filter(Boolean)))
      .map((value) => ({ value, count: 0 }))
      .sort((a, b) => String(a.value).localeCompare(String(b.value), 'ru'));
    const scopedGroups = selectedCategory
      ? (this.categoryGroupsIndex.get(selectedCategory) || [])
      : allGroups;
  ...
  populateBrandTaxonomySelectors() {
    ...
    if (createBrandSelect) {
      ...
      (this.catalogTaxonomy.brands || []).forEach((b) => {
        createBrandSelect.add(new Option(String(b.name || ''), String(b.id || '')));
      });
    }

    const labelById = this.getBrandCategoryLabelMap();
    if (productBrandCategories) {
      const selected = new Set(Array.from(productBrandCategories.selectedOptions).map((o) => o.value));
      productBrandCategories.innerHTML = '';
      (this.catalogTaxonomy.brandCategories || []).forEach((c) => {
        const id = String(c.id || '');
        const label = labelById.get(Number(c.id)) || String(c.name || id);
        const opt = new Option(label, id);
        opt.selected = selected.has(id);
        productBrandCategories.add(opt);
      });
    }
    if (productPrimaryBrandCategory) {
      const selected = String(productPrimaryBrandCategory.value || '');
      productPrimaryBrandCategory.innerHTML = '<option value="">\u041d\u0435 \u0432\u044b\u0431\u0440\u0430\u043d\u0430</option>';
      (this.catalogTaxonomy.brandCategories || []).forEach((c) => {
        const id = String(c.id || '');
        const label = labelById.get(Number(c.id)) || String(c.name || id);
        const opt = new Option(label, id);
        opt.selected = selected === id;
  ...
  renderTaxonomyTrees() {
    ...
    if (brandsBody) {
      ...
      brandsBody.innerHTML = rows.length
        ? rows.map((b) => `
          <tr>
            <td>
              #${b.id} ${this.escapeHtml(b.name || '')}
              <span style="float:right;display:flex;gap:6px;">
                <button class="btn btn-outline btn-xs" onclick="editBrand(${b.id})">Ред</button>
                <button class="btn btn-outline btn-xs" onclick="deleteBrand(${b.id})">Удалить</button>
              </span>
            </td>
          </tr>
        `).join('')
        : '<tr><td style="padding:16px;color:#64748b;">Брендов пока нет</td></tr>';
    }
    ...
    if (brandBody) {
      ...
      for (const [brandId, categories] of grouped.entries()) {
        ...
        parts.push(this.renderCategoryTreeRows(
          categories,
          (x) => Number(x.parentId || 0),
          1,
          'brandCategory'
      ...
      brandBody.innerHTML = parts.length ? parts.join('') : '<tr><td style="padding:16px;color:#64748b;">Брендовых категорий пока нет</td></tr>';
    }
  }
      ...
      return list
        .map((row) => {
          ...
          const actionButtons = actionType === 'functional'
            ? `<span style="float:right;display:flex;gap:6px;"><button class="btn btn-outline btn-xs" onclick="editFunctionalCategory(${row.id})">Ред</button><button class="btn btn-outline btn-xs" onclick="deleteFunctionalCategory(${row.id})">Удалить</button></span>`
            : actionType === 'brandCategory'
              ? `<span style="float:right;display:flex;gap:6px;"><button class="btn btn-outline btn-xs" onclick="editBrandCategory(${row.id})">Ред</button><button class="btn btn-outline btn-xs" onclick="deleteBrandCategory(${row.id})">Удалить</button></span>`
              : '';
          const me = `<tr><td style="padding-left:${pad}px;">#${row.id} ${this.escapeHtml(row.name || '')}${actionButtons}</td></tr>`;
          const kids = render(row.id, depth + 1);
          return `${me}${kids}`;
        })
        .join('');
    };
        ...
        <tr>
          ...
          <td>
            <div class="action-buttons">
              <a href="#" class="btn-action" onclick="viewOrderDetails('${o.id}'); return false;">Детали</a>
              <a href="#" class="btn-action" onclick="updateOrderStatus('${o.id}'); return false;">Статусы</a>
            </div>
          </td>
        </tr>
      `).join('');
  ...
  renderOrderDocumentsRows(documents = []) {
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="3" style="padding:8px 4px;color:#6b7280;">Файлы не добавлены</td></tr>';
      return;
    }
    body.innerHTML = rows.map((doc, index) => `
      <tr>
        <td><input class="input order-doc-title" type="text" value="${this.escapeHtml(doc.title || '')}" placeholder="Инструкция" /></td>
  ...
  renderProductsView() {
    ...
    view.forEach((product) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          ${product.image ? `<img src="${product.image}" alt="${product.name}" class="product-photo">` : '<div class="product-photo">рџ“¦</div>'}
        </td>
        <td>
        ...
        <td>
          <div class="action-buttons">
            <a href="#" class="btn-action" onclick="editProduct('${product.id}'); return false;">Редакт.</a>
            <a href="/#/product/${product.id}" class="btn-action" target="_blank">Смотреть</a>
          </div>
        </td>
      `;
  ...
  getSelectedFunctionalCategories() {
    const select = document.getElementById('productFunctionalCategories');
    if (!select) return [];
  ...
  setSelectedFunctionalCategories(values) {
    const select = document.getElementById('productFunctionalCategories');
    if (!select) return;
    const set = new Set((Array.isArray(values) ? values : []).map((x) => String(x || '').trim()).filter(Boolean));
    Array.from(select.options).forEach((opt) => {
      opt.selected = set.has(String(opt.value || '').trim());
    });
  }

  ensureFunctionalCategorySelected(value) {
    const select = document.getElementById('productFunctionalCategories');
    const target = String(value || '').trim();
    if (!select || !target) return;
    let option = Array.from(select.options).find((opt) => String(opt.value || '').trim() === target);
    if (!option) {
      option = new Option(target, target);
      select.add(option);
    }
  ...
  updateHeaderFilterBadges() {
    ...
    map.forEach(([id, key]) => {
      ...
      if (key === 'status') {
        const labels = { active: 'Активный', draft: 'Черновик', hidden: 'Скрыт', archived: 'Архив', extra: 'Лишний' };
  ...
  getStatusBadge(product) {
    ...
    const raw = String(product.status || 'active').toLowerCase();
    if (raw === 'draft') return { css: 'draft', text: 'Draft' };
    if (raw === 'hidden') return { css: 'hidden', text: 'Hidden' };
    if (raw === 'archived') return { css: 'hidden', text: 'Archived' };
    return { css: 'active', text: 'Active' };
  }
  ...
  fillProductForm(product) {
    ...
    this.ensureFunctionalCategorySelected(primaryFunctionalCategory);
    document.getElementById('productGroup').value = groupValue;
    const brandCategorySelect = document.getElementById('productBrandCategories');
    const primaryBrandCategorySelect = document.getElementById('productPrimaryBrandCategory');
    const selectedBrandCategoryIds = new Set((product.brandCategoryIds || []).map((x) => String(x)));
    if (brandCategorySelect) {
      Array.from(brandCategorySelect.options).forEach((opt) => {
        opt.selected = selectedBrandCategoryIds.has(String(opt.value));
      });
    }
    if (primaryBrandCategorySelect) {
      primaryBrandCategorySelect.value = product.primaryBrandCategoryId ? String(product.primaryBrandCategoryId) : '';
    }
  ...
  async saveProduct() {
    ...
    const primaryBrandCategoryId = Number(document.getElementById('productPrimaryBrandCategory')?.value || 0) || null;
    ...
    if (!functionalCategories.length) {
      this.showError('Выберите минимум одну функциональную категорию');
      return;
    }
    if (!brandCategoryIds.length) {
      this.showError('Выберите минимум одну брендовую категорию');
      return;
    }
  ...
  setMedia(media) {
    this.currentMedia = (Array.isArray(media) ? media : []).map((item, index) => ({
      id: item?.id ?? null,
  ...
  renderMediaTable() {
    ...
    if (!this.currentMedia.length) {
      tbody.innerHTML = '<tr><td colspan="6" class="variants-empty">Фото пока нет</td></tr>';
      this.updateMediaCount();
      return;
    }
    this.currentMedia.forEach((m, index) => {
      ...
      tr.innerHTML = `
        ...
        <td><button class="btn btn-outline" onclick="deleteMediaRow(${index})">Удалить</button></td>
      `;
      tbody.appendChild(tr);
    });
    this.updateMediaCount();
  ...
  async createBrandCategory() {
    ...
    const parentIdRaw = document.getElementById('newBrandCategoryParentId')?.value || '';
    if (!brandId || !name) {
    ...
    try {
      ...
      const parentInput = document.getElementById('newBrandCategoryParentId');
      if (nameInput) nameInput.value = '';
      if (parentInput) parentInput.value = '';
      await this.loadTaxonomyDictionaries();
  ...
  async editFunctionalCategory(categoryId) {
    ...
    const name = String(nextName).trim();
    if (!name) return;
    const parentIdRaw = prompt('Parent ID (пусто = корень)', row.parentId == null ? '' : String(row.parentId));
  ...
  previewProduct() {
    if (this.currentProduct?.id) {
      window.open(`/#/product/${this.currentProduct.id}`, '_blank');
    }
  }

  duplicateProduct() {
    this.showError('Дублирование пока не реализовано');
  }

  archiveProduct() {
    this.showError('Архивация пока не реализована');
  }
  ...
  refreshQualityIndicator() {
    ...
    if (hasConflict) {
      holder.innerHTML = '<div class="quality-status warning">Product is active but has no active variants</div>';
      return;
    }
    holder.innerHTML = '<div class="quality-status ready">Ready for publish</div>';
  }
...
function callApp(method, ...args) {
  if (!window.adminApp || typeof window.adminApp[method] !== 'function') return;
...
window.applyOrdersFilters = () => callApp('applyOrdersFilters');
